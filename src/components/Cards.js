...
const Cards = () => {
  ...
  // here we are using useEffect to fetch the data from the database
  useEffect(() => {
  ...
  return (
    ...
    { loading ? <div className='w-full flex justify-center items-center h-96'><TailSpin height={90} color='white' /></div> :
     data.map((e,i) => {
      return (
      ...
      <div key={i} className='card font-medium shadow-lg p-2 hover:-translate-y-3 cursor-pointer mt-6 transition-all duration-500  '>
          <img className='h-90 md:h-72' src={e.image} alt="" />
